...
class NewsCoordinator(DataUpdateCoordinator[list[dict]]):
            ...
            if new_counts:
                ...
                if self.hass.services.has_service(
                    ...
                    await self.hass.services.async_call(
                        ...
                        {
                            "notification_id": (
                                f"news_{self.entry.entry_id}_summary"
                            ),
                            "title": "News: New Articles",
                            "message": message,
                        },
                        blocking=True,
                    )
    ...
    def _get_timestamp(self, value):
        """Convert a string or datetime to a timestamp for sorting."""
        if not value:
            return 0
    ...
    def _fetch(self):
        ...
        for feed_name, url in self.feeds_data.items():
            ...
            for entry in parsed.entries[: self.articles_per_feed * 2]:
                entries.append(
                    {
        ...
        # ---------------- P2000 SENSOR ---------------- #
        state = self.hass.states.get("sensor.p2000_meldingen")

        if state:
            meldingen = state.attributes.get("meldingen")

            if meldingen:
                for msg in meldingen:
                    entries.append(
                        {
                            "title": msg.get("title", ""),
                            "link": msg.get("link", ""),
                            "entity_picture": "/local/media/p2000.png",
                            "feed_name": "P2000",
                            "summary": msg.get("description", ""),
                            "published": msg.get("datetime"),
                            "p2000_id": msg.get("id"),
                        }
                    )
        ...
        entries.sort(
            key=lambda e: self._get_timestamp(e.get("published")),
            reverse=True,
        )
        ...
        for article in entries:
            ...
            if feed == "P2000":
                if p2000_count < 5:
                    final_entries.append(article)
                    p2000_count += 1
                continue

            # Handle RSS feeds
            count = feed_counters.get(feed, 0)
            if count < self.articles_per_feed:
                final_entries.append(article)
                feed_counters[feed] = count + 1
        self._cached_entries = final_entries
        return final_entries

    # ---------------- QUEUE LOGIC ---------------- #

    def _article_id(self, article) -> str:
        # Prefer real ID if available (P2000)
        if article.get("p2000_id"):
            return article["p2000_id"]
    ...
    def _enqueue_new_articles(self, entries: list[dict]):
        for article in entries:
            ...
            published_ts = self._get_timestamp(article.get("published"))
    ...
    def _dwains_listener(self, event):
        ...
        article_id = f"news_{self.entry.entry_id}"
        summary_id = f"news_{self.entry.entry_id}_summary"
