...
async def fetch_feeds(hass):
    session = async_get_clientsession(hass)
    try:
        async with session.get(FEEDS_URL, timeout=15) as resp:
            text = await resp.text()
    except Exception as err:
        _LOGGER.error("Failed to fetch feeds page: %s", err)
        return {}
...
class NewsConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    VERSION = 3

    async def async_step_user(self, user_input=None):
        errors = {}
        feeds = await fetch_feeds(self.hass)
        ...
        if user_input:
            ...
            await self.async_set_unique_id("news")
        ...
        schema = vol.Schema(
            {
                vol.Required(
                    ...
                    selector.SelectSelectorConfig(
                        options = [{"value": v, "label": v} for v in feeds_select],
                ...
                vol.Optional(
                    ...
                    selector.SelectSelectorConfig(
                        options = [{"value": v, "label": v} for v in inclusions],
                        mode=selector.SelectSelectorMode.DROPDOWN,
                        multiple=True,
                    )
                ),
    ...
    def async_get_options_flow(entry):
        return NewsOptionsFlow(entry)


class NewsOptionsFlow(config_entries.OptionsFlow):
        ...
        schema = vol.Schema(
            {
                ...
                vol.Optional(
                    ...
                    selector.SelectSelectorConfig(
                        options = [{"value": v, "label": v} for v in inclusions_available],
                        mode=selector.SelectSelectorMode.DROPDOWN,
                        multiple=True,
                    )
                ),
